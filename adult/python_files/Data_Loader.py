...
class Data_Loader:
    vorverarbeitet_path = "dataset/vorverarbeitet/"
    scenario6_path = "dataset/szenario6/"
    sceanrio_3_file_path = "dataset/szenario3/vorverarbeitet.csv"
    generalized_train_path = "dataset/manipuliert/adult_train.csv"
    generalized_test_path = "dataset/manipuliert/adult_test.csv"
    normal_path = "dataset/"
    CATEGORICAL_COLUMNS = ['workclass', 'education', 'marital-status', 'occupation', 'relationship', 'race', 'sex', 'native-country']

    def __init__(self, szeanrio: Szenario):
        self.data_train_original = pd.read_csv('dataset/adult_train.csv')
        self.data_test_original = pd.read_csv('dataset/adult_test.csv')
    ...
    #Funktion, die die originalen Spalten mit bestimmten vorverarbeiteten Spalten ersetzt
    def replace_columns_with_preprocessed(self, columns: list[Spalten]):
        for column in columns:
            colum_df = pd.read_csv(self.vorverarbeitet_path + column.value.name + "_vorverarbeitet.csv")
            self.data_train.drop(columns=[column.value.name], inplace=True)
                        
            groups1 = self.data_train.groupby('record_id')
            groups2 = colum_df.groupby('record_id')
            result = pd.DataFrame()

            for record_id, group1 in groups1:
                if record_id in groups2.groups:
                    group2 = groups2.get_group(record_id)
                    temp_result = self.cartesian_product(group1, group2)
    ...
    def preprocess_scenario_1_and_2(self, columns: list[Spalten], manipulate_test_data: bool):
        for column in columns:
            preprocessed_df = pd.read_csv(self.vorverarbeitet_path + column.value.name + "_vorverarbeitet.csv")
    ...
    def set_types(self):
        ...
        for column in data_original.columns:
            if self.szenario == Szenario.Szenario3 or self.szenario == Szenario.Szenario5:
                if column != "record_id" and column != "income":
